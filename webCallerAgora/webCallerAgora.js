// webCallerAgora/webCallerAgora.js
...
/**
 * Generate RTC token for web caller video/audio calling
 ...
 */
export const generateWebCallerRtcToken = (channelName, uid, role = 'publisher', expirationTime = DEFAULT_EXPIRATION_TIME) => {
...
/**
 * Generate RTM token for web caller real-time messaging (in-call chat)
 * @param {string} userId - User ID for RTM
 * @param {number} expirationTime - Token expiration in seconds
 * @returns {string} RTM token
 */
export const generateWebCallerRtmToken = (userId, expirationTime = DEFAULT_EXPIRATION_TIME) => {
  if (!AGORA_APP_ID || !AGORA_APP_CERTIFICATE) {
    throw new Error('Agora credentials not configured');
  }
